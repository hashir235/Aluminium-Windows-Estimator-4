...
int SGM_Win_f::windowCount = 0; 
void SGM_Win_f::resetWindowCount() { windowCount = 0;}
SGM_Win_f::SGM_Win_f() {
    height = 0.0;
    width = 0.0;
    collarType = 0;
...
bool SGM_Win_f::inputWindowData(bool isEditing) {
    if (!isEditing) {  
        label = "SGM_Win_f #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
void SGM_Win_f::calculateSections() {
    sectionMap.clear();
...
double SGM_Win_f::getAreaSqFt() const {
    return 0.0;  // Not applicable for F-series
}

std::string SGM_Win_f::getLabel() const {
    return label;
}

const SectionMap& SGM_Win_f::getSectionMap() const {
    return sectionMap;
}


void SGM_Win_f::sendToEstimator(EstimateLengthManager& estimator) {
    estimator.addWindowSections(label, sectionMap);
}


double SGM_Win_f::getHeight() const {  return height;}
double SGM_Win_f::getWidth() const { return width;}
int SGM_Win_f::getCollarType() const { return collarType; }
int SGM_Win_f::getLockType() const { return lockType; }  // ✔ New getter for lock type

void SGM_Win_f::sendToEstimator() {
    std::cout << "[SGM_Win_f] Warning: No estimator provided in sendToEstimator().\n";
}

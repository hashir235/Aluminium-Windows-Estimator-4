...
def draw_header(canv, doc):
    """Draw circular logo at the top center."""
    ...
    try:
        ...
        canv.drawImage(temp_path, logo_x, logo_y, width=60, height=60, mask='auto')
...
def draw_footer(canv, doc):
    ...
    footer_text = "Thank you for using <b>AL Window Solver</b> — Aluminium Estimation Made Easy."
...
def decorate_page(canv, doc):
    ...
    # ✅ Watermark on top layer (after all)
    draw_watermark(canv)

# ---------------- MAIN FUNCTION ----------------
def generate_material_pdf():
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    pdf_filename = f"Material_Table_{timestamp}.pdf"

    with open("Temp_material_table.txt", "r", encoding="utf-8") as f:
    ...
    # ✅ Smaller topMargin so content starts earlier
    ...
    elements = []
    # ❌ Removed large spacer (was pushing to next page)
    ...
    # Light blue grouping background + merge logic
    start_row = 1
    ...
    elements.append(Paragraph(f"<b>Generated on:</b> {datetime.now().strftime('%d-%b-%Y %H:%M')}", styles['Normal']))


    # Build PDF with decorator

...
void addWindow(vector<unique_ptr<FrameComponent>>& windows,
    ...
    for (const auto& [id, typePair] : EstimatorWindowTypes) {
    ...
    if (!cin || EstimatorWindowTypes.find(choice) == EstimatorWindowTypes.end()) {
    ...
    auto newWin = EstimatorWindowTypes.at(choice).second();

...
bool AutoRatesManager::checkAndUpdateRates() {
    ...
    else std::cout << " Could not update rates from server.\n";
    return success;
}
...
void AutoRatesManager::selectOptions() {
    ...
    // Color selection
    std::vector<std::string> colorNames = {"DULL", "H23/PC-RAL (Champagne)", "SAHRA/BRN", "BLACK/MULTI", "WOOD COAT"};
...
double AutoRatesManager::getRateForSection(const string& sectionName) const {
    ...
    std::cerr << " Warning: Section \"" << sectionName << "\" not found in rates. Using 0.0 rate.\n";
    return 0.0;
}
...
void AutoRatesManager::reviewAndEditRates(vector<SectionSummary>& sections) {
    if (sections.empty()) {
        std::cout << " No sections to review.\n";
        return;
    }
    ...
    while (true) {
        ...
        if (choice < 1 || choice > (int)sections.size()) {
            std::cout << " Invalid line number. Try again.\n";
            continue;
        }
        ...
        if (!std::cin) {
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            std::cout << " Invalid rate. Operation cancelled for this line.\n";
            continue;
        }
        if (newRate < 0) {
            std::cout << " Rate cannot be negative. Operation cancelled.\n";
            continue;
        }
...
void AutoRatesManager::generateMaterialTxt(const vector<SectionSummary>& sections) {
    ...
    if (!file.is_open()) {
        cerr << "Error: Could not create Temp_material.txt\n";
        return;
    }

    double grandTotal = 0.0;

    // SAME STRUCTURE — nothing changed
    ...
    cout << "Temp_material.txt overwritten successfully!\n";
}

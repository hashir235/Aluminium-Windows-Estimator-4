...
void SM_Win_e::resetWindowCount() { windowCount = 0;} 
int SM_Win_e::windowCount = 0;
SM_Win_e::SM_Win_e() {
    height = 0.0;
    width = 0.0;
    collarType = 0;
...
bool SM_Win_e::inputWindowData(bool isEditing ) {
    if (!isEditing) {
        
        label = "SM_Win_e #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
void SM_Win_e::calculateSections() {
    sectionMap.clear();
...
double SM_Win_e::getAreaSqFt() const {
    return height * width;
}

std::string SM_Win_e::getLabel() const {
    return label;
}

const SectionMap& SM_Win_e::getSectionMap() const {
    return sectionMap;
}

void SM_Win_e::sendToEstimator(EstimateLengthManager& estimator) {
...
void SM_Win_e::sendToEstimator() {
    std::cout << "[SM_Win_e] Warning: No estimator provided.\n";
}

...
int O_Win_f::windowCount = 0; 
void O_Win_f::resetWindowCount() { windowCount = 0;}
O_Win_f::O_Win_f() {
    height = 0.0;
    width = 0.0;
    collarType = 0;
...
bool O_Win_f::inputWindowData(bool isEditing) {
    if (!isEditing) {

        label = "O_Win_f #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
void O_Win_f::calculateSections() {
    sectionMap.clear();
...
switch (collarType) {
        ...
        addSection("D50A", { labelWith("HL", ((h - 4)+ cm)/ feet), labelWith("HR", ((h - 4)+ cm)/ feet ), labelWith("WT",((w - 4)+ cm)/ feet ), labelWith("WB", ((w - 4)+ cm)/ feet ) });

    if(netChoice == "y" || netChoice == "Y") {
        addSection("D29", { labelWith("HL", (h + cm)/ feet ), labelWith("HR", (h + cm)/ feet ), labelWith("WT", (w + cm)/ feet ), labelWith("WB", (w + cm)/ feet ) });
    }
    
}

double O_Win_f::getAreaSqFt() const {
    return 0.0;  // Not applicable for F-series
}

std::string O_Win_f::getLabel() const {
    return label;
}

const SectionMap& O_Win_f::getSectionMap() const {
    return sectionMap;
}


void O_Win_f::sendToEstimator(EstimateLengthManager& estimator) {
...
void O_Win_f::sendToEstimator() {
    std::cout << "[O_Win_f] Warning: No estimator provided in sendToEstimator().\n";
}

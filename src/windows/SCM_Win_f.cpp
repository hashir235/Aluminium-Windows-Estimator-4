...
void SCM_Win_f::resetWindowCount() { windowCount = 0;} 
int SCM_Win_f::windowCount = 0; 
SCM_Win_f::SCM_Win_f()
...
bool SCM_Win_f::inputWindowData(bool isEditing) {
    if (!isEditing) {
...
bool SCM_Win_f::selectWindowType() {
    while (true) {
        std::cout << "\n------ Select Window Type ------\n"
                  << "[1] Center Fix\n"
                  << "[2] Center Slide\n"
...
///////////////////////// Calculations Method /////////////////////////
void SCM_Win_f::calculateSections() {
    sectionMap.clear();
    ...
    if (collarType == 1) {
        ...
        addSection("M26F", { labelWith("WB_l", ((wl + 6)+ cm ) / feet), labelWith("WB_r", ((wr + 6)+ cm ) / feet) });
    }
        ...
        addSection("M26", { labelWith("WB_l", (wl + cm ) / feet), labelWith("WB_r", (wr + cm ) / feet) });
    }
...
double SCM_Win_f::getAreaSqFt() const {
    return 0.0;  // Not applicable for F-series
}


std::string SCM_Win_f::getLabel() const {
    return label;
}

const SectionMap& SCM_Win_f::getSectionMap() const {
    return sectionMap;
}


void SCM_Win_f::sendToEstimator(EstimateLengthManager& estimator) {
...
double SCM_Win_f::getLeftWidth() const { return leftWidth; }
int SCM_Win_f::getCollarType() const { return collarType; }


void SCM_Win_f::sendToEstimator() {

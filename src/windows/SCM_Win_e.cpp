...
void SCM_Win_e::resetWindowCount() { windowCount = 0;} 
int SCM_Win_e::windowCount = 0; 
SCM_Win_e::SCM_Win_e()
    : height(0.0), width(0.0), rightWidth(0.0), leftWidth(0.0),
      collarType(0), winType(0), inputInInches(false) {++windowCount;}


bool SCM_Win_e::inputWindowData(bool isEditing) {
    if (!isEditing) {
        
        label = "SCM_Win_e #" + std::to_string(windowCount) + " -> ";
    } else {
        std::cout << "\n--- Editing " << label << " ---\n";
    }
...
bool SCM_Win_e::selectWindowType() {
    while (true) {
        std::cout << "\n------ SCM_Win_e Options ------\n"
                  << "1. Center Fix\n"
                  << "2. Center Slide\n"
...
/////////////////////////// CALCULATIONS ///////////////////////////
void SCM_Win_e::calculateSections() {
    sectionMap.clear();
...
double SCM_Win_e::getAreaSqFt() const {
    return height * width;
}

std::string SCM_Win_e::getLabel() const {
    return label;
}

const SectionMap& SCM_Win_e::getSectionMap() const {
    return sectionMap;
}


void SCM_Win_e::sendToEstimator(EstimateLengthManager& estimator) {
    estimator.addWindowSections(label, sectionMap);
}

char SCM_Win_e::getSize() const {return unitChoice;}
double SCM_Win_e::getHeight() const { if (inputInInches) {return height * 12.0;} return height;}
double SCM_Win_e::getWidth() const {if (inputInInches){return width * 12.0;} return width;}
double SCM_Win_e::getRightWidth() const { if (inputInInches) {return rightWidth * 12.0;} return rightWidth; }
double SCM_Win_e::getLeftWidth() const { if (inputInInches) {return leftWidth * 12.0;} return leftWidth; }
int SCM_Win_e::getCollarType() const { return collarType; }


void SCM_Win_e::sendToEstimator() {
